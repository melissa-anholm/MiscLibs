...
// MultiTuple.cpp (v. 14):
// 1.June.2021:
// Done in parallel with experimental MultiTuple.cpp.
...
//	string matched_runset = "";
	...
	if(argc>=5)
	{
		threshold_index      = atoi(argv[3]);  // SNR.  indices:  0,1,2,3,4.
		sigma_cut            = atof(argv[4]);  // agreement.
		//
	...
	TObjString * params_string = new TObjString(additional_filename_info.c_str());
	if(is_g4) { cout << "We are using simulated data." << endl; } 
	else      { cout << "We are using experimental data." << endl; }
	//	
	cout << "ReTuple version " << version << endl;
//	cout << "We will analyze Run " << runno << " as a part of Data Set " << matched_runset << "." << endl;   // 
...
//	cout << "For BB1s, we use an energy threshold of " << bb1_energy_threshold << " keV." << endl;
	...
	Double_t upper_E_B;
	Double_t lower_E_B;
	Double_t upper_E_res_B;
	Double_t lower_E_res_B;
	//
	Double_t upper_E_CD;
	Double_t lower_E_CD;
	Double_t upper_E_res_CD;
	Double_t lower_E_res_CD;
	//
	if(!is_g4)
	{
	...
	{
		TBranch *upper_e_b_B          = friend_tree -> Branch("upper_scint_E_B",     &upper_E_B);
		TBranch *lower_e_b_B          = friend_tree -> Branch("lower_scint_E_B",     &lower_E_B);
...
//	TBranch * N_hits_bb1_b_branch = friend_tree -> Branch("N_hits_bb1_b", &N_hits_bb1_b);
	...
	//	cout << "entry is got." << endl;
		...
		// scint. calibration, SOE TOF walk correction:
		if(!is_g4)
		{
	...
	//	cout << "or here?" << endl;
		...
		// Set up event types:
		is_normal_t_20 = kFALSE;
		is_normal_b_20 = kFALSE;
		is_normal_t_30 = kFALSE;
		is_normal_b_30 = kFALSE;
		is_normal_t_40 = kFALSE;
		is_normal_b_40 = kFALSE;
		is_normal_t_50 = kFALSE;
		is_normal_b_50 = kFALSE;
		is_normal_t_60 = kFALSE;
		is_normal_b_60 = kFALSE;
		is_normal_t_70 = kFALSE;
		is_normal_b_70 = kFALSE;
		is_normal_t_80 = kFALSE;
